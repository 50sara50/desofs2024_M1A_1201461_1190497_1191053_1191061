...
exports.Hyper = {
     ...
     * @param {String[]} [filter] -
     */
    item: function (dataObj, linksBuilder, userRole,subscriptionID,filter, links) {
        if (!filter) {
            filter = FILTER;
        }
        return {
            data: dataObj, // _.omit(data.toObject ? data.toObject() : data, filter),
            links: linksBuilder(dataObj,userRole,subscriptionID,links)
        };
    },
     ...
     * @param {*} queries
     */
    collection: function (data, itemLinksBuilder, colLinks, template, queries, id) {
        var self = this;
        var hyperData = _.map(data, (i) => self.item(i, itemLinksBuilder,null,id));
