...
namespace StreamingService.Test.Services
{
    ...
    public class PlanServiceTest
    {
        ...
        public async Task GetPlanNonexistentPlanNameReturnsNull()
        {
            ...
            IUnitOfWork unitOfWork = new UnitOfWork(context);
            PlanService planService = new(unitOfWork);
            PlanResponse? response = await planService.GetPlan(planName);
        ...
        public async Task GetPlanEmptyPlanNameThrowsArgumentException()
        {
            ...
            IUnitOfWork unitOfWork = new UnitOfWork(context);
            PlanService planService = new(unitOfWork);
            ArgumentException exception = await Assert.ThrowsExceptionAsync<ArgumentException>(async () => await planService.GetPlan(""));
        ...
        [TestMethod]
        public async Task GetPlansWithPaginationReturnsPaginatedResults()
        {
            if (this.context == null)
            {
                Assert.Fail("Context is null");
            ...
            PagedResponseDTO<PlanResponse> response = await planService.GetPlans(pageSize, currentPage);
        ...
        [TestMethod]
        public async Task GetPlanWithoutPaginationReturnsAllRecords()
        {
            if (this.context == null)
            {
                Assert.Fail("Context is null");
            ...
            CreatePlanContract plan2 = new() { PlanName = "New Plan 2", MonthlyFee = 15, NumberOfMinutes = 1500 };
            await planService.CreatePlan(plan2);
            IEnumerable<PlanResponse> response = await planService.GetPlans();
