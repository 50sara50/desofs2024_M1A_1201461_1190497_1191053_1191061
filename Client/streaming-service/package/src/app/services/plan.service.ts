...
export class PlanService {
  ...
  createPlan(planName: string, monthlyFee: number, numberOfMinutes: number): Observable<any> {
    ...
    return this.httpClient.post(this.Url, body, { headers: headers }).pipe(map(this.extractData));

  }

  createValidPlan(planName: string, monthlyFee: number, numberOfMinutes: number): Observable<any> | null {
  ...
  validateData(planName: string, monthlyFee: number, numberOfMinutes: number): boolean {
    if (planName == null) {
